...
int WriteIndex(MemIndex *mi, DocTable *dt, const char *file_name) {
  ...
  int hd_bytes = WriteHeader(f, dt_bytes, mi_bytes);
  if (hd_bytes == kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }
...
static int WriteHashTable(FILE *f,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int bucket_bytes = WriteHTBucket(f, bucket_pos, chain, fn);
...
static int WriteDocidToDocnameFn(FILE *f,
  ...
  if (fwrite(docname, sizeof(char), file_name_bytes, f) != file_name_bytes) {
    return kFailedWrite;
  }
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  uint16_t word_bytes = strlen(wp->word);
